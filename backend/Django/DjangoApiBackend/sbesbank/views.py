...
def CreateAccount(request):
    ...
    print(account_data['clientId'])
    print(account_data['currency'])
...
def DoExchangeTransfer(exchangeData):
    ...
    try:
        ...
        if str(accountT.currency)!='RSD':
            try:
                print(accountT.currency)

                currFrom = ExchangeRate.objects.get(currency = accountT.currency)
            except:
                print("OVDE SAM SE SJEBO1")

            convertedInDinars = amount*currFrom.rateInDinar
        else:
            convertedInDinars = amount
        
        print(convertedInDinars)
        if str(accountF.currency)!='RSD':
            try:
                currTo = ExchangeRate.objects.get(currency = accountF.currency)
            except:
                print("OVDE SAM SE SJEBO2")
        ...
        myAccInfo = CreateModel(TrMyAccountInfo,{
            'id':GetNextId(TrMyAccountInfo),
            'balanceBefore':accountF.accountBalance,
            'balanceAfter':accountF.accountBalance+convertedValue,
            'accountNumber':accountFrom,
            'billingAddress':userEx.billingAddress,
            'fullName':userEx.fullName
        })
        ...
        accountF.accountBalance = accountF.accountBalance + convertedValue
        accountF.save()
        serializer = TransactionSerializer(transaction) 
...
def CheckCurrency(request):
    ...
    try:
        ...
        if account!=None:
            ...
            return JsonResponse({"Currency":"Account with that accountNumber doesn't exist"})
    except:
        return JsonResponse({"Currency":"Account with that accountNumber doesn't exist"})
